...
class MyMemoryTranslatorExt(BaseTranslator):
    """
        ...
        if is_input_valid(text, max_chars=500):
            ...
            for s in  text.split('\n'):
                ...
                if -1 == pos:
                    ...
                    textArr.append(s[:pos])
                    textTag.append(s[pos:])
            ...
            if translation:
                if not return_all:            
                    return '\n'.join([ a + b  for a,b in zip(translation.split('\n'),textTag)])
                else:
...
def selectService(service):
    if service:
        class_translate = getattr(deep_translator, service) if service in dir(deep_translator) else MyMemoryTranslatorExt #getattr(deep_translator, "")
        langs_support = {}
        auto_support= True
        ...
        if service in ("MyMemoryTranslator", "LingueeTranslator", "MyMemoryTranslatorExt"):
...
def service_translate(service, text, from_translate="auto", to_translate="en", prop_data={}):
        ...
        # MyMemoryTranslator
        elif service == "MyMemoryTranslatorExt":
            translated = MyMemoryTranslatorExt(source=from_translate, target=to_translate, proxies=proxyes).translate(text)
...
def deep_translator_function(from_translate, to_translate, add_proxies, proxies, auth_data, service, text, lang_support={}):
        ...
        try:
            if text:          
                ...
                # Detect language
                tServices = ("DeeplTranslator", "QcriTranslator", "LingueeTranslator", "PonsTranslator", "PapagoTranslator", "BaiduTranslator", "MyMemoryTranslator", "MyMemoryTranslatorExt")
